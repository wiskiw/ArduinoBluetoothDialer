...
public class BitUtilsTest {
    ...
    public void testBitShiftLeft() {
        ...
        byte exp = (byte) 0b11010000;

        byte result = BitUtils.bitShiftLeft(src, (byte) 4);
...
//        logSrcExpResBytes(src, exp, result);
    ...
    public void testIsBitUp() {
        ...
        assertTrue(BitUtils.isBitUp(src1, 0));
        assertTrue(BitUtils.isBitUp(src1, 1));
        assertTrue(BitUtils.isBitUp(src1, 2));
        assertFalse(BitUtils.isBitUp(src1, 3));
        assertTrue(BitUtils.isBitUp(src1, 4));
        assertTrue(BitUtils.isBitUp(src1, 5));
        assertFalse(BitUtils.isBitUp(src1, 6));
        assertFalse(BitUtils.isBitUp(src1, 7));

        byte src2 = (byte) 0b11111001;
        assertTrue(BitUtils.isBitUp(src2, 0));
        assertTrue(BitUtils.isBitUp(src2, 1));
        assertTrue(BitUtils.isBitUp(src2, 2));
        assertTrue(BitUtils.isBitUp(src2, 3));
        assertTrue(BitUtils.isBitUp(src2, 4));
        assertFalse(BitUtils.isBitUp(src2, 5));
        assertFalse(BitUtils.isBitUp(src2, 6));
